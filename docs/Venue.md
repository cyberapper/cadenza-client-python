# Venue

Trading venue identifier. Sandbox environments are treated as separate venues.

## Enum

* `EMPTY` (value: `''`)

* `BINANCE` (value: `'BINANCE'`)

* `B2C2` (value: `'B2C2'`)

* `WINTERMUTE` (value: `'WINTERMUTE'`)

* `BLOCKFILLS` (value: `'BLOCKFILLS'`)

* `STONEX` (value: `'STONEX'`)

* `CUMBERLAND` (value: `'CUMBERLAND'`)

* `IBKR` (value: `'IBKR'`)

* `DERIBIT` (value: `'DERIBIT'`)

* `BITFINEX` (value: `'BITFINEX'`)

* `OKX` (value: `'OKX'`)

* `BINANCE_SANDBOX` (value: `'BINANCE_SANDBOX'`)

* `B2_C2_UAT` (value: `'B2C2_UAT'`)

* `WINTERMUTE_UAT` (value: `'WINTERMUTE_UAT'`)

* `BLOCKFILLS_UAT` (value: `'BLOCKFILLS_UAT'`)

* `STONEX_UAT` (value: `'STONEX_UAT'`)

* `CUMBERLAND_UAT` (value: `'CUMBERLAND_UAT'`)

* `IBKR_PAPER` (value: `'IBKR_PAPER'`)

* `DERIBIT_UAT` (value: `'DERIBIT_UAT'`)

* `BITFINEX_UAT` (value: `'BITFINEX_UAT'`)

* `OKX_UAT` (value: `'OKX_UAT'`)

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


